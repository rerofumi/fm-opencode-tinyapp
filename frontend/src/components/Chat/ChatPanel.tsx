...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  selectedAgent,
  onPilotStatusChange,
  pilotStatus: externalPilotStatus,
}) => {
  ...
  const handledQuestionPartIDsRef = useRef<Set<string>>(new Set());
  ...
  const normalizeQuestionOptions = (raw: any): QuestionOption[] => {
    ...
    return raw
      .map((item): QuestionOption | null => {
        ...
        if (item && typeof item === "object") {
          ...
          const value = String(
            item.value ?? item.id ?? item.key ?? item.response ?? label,
          );
  ...
  const firstNonEmptyOptions = (...candidates: any[]): QuestionOption[] => {
    for (const candidate of candidates) {
      const options = normalizeQuestionOptions(candidate);
      if (options.length > 0) return options;
    }
    return [];
  };

  const maybeOpenQuestionModal = (part: any) => {
    if (!part || part.type !== "tool" || part.tool !== "question") return;
    if (handledQuestionPartIDsRef.current.has(part.id)) return;
    if (questionModal?.partID === part.id) return;
    ...
    const detailRaw =
      input.description ?? input.detail ?? input.message ?? metadata.detail;
    const detail = detailRaw ? String(detailRaw) : undefined;
    ...
    setQuestionModal({
      ...
      permissionID:
        metadata.permissionID ?? state.permissionID ?? input.permissionID,
      controlTemplate: input.body ?? state.body ?? metadata.body,
    });
  };

  const scanPendingQuestionFromMessages = (msgs: any[]) => {
    const assistantMessages = [...msgs]
      .reverse()
      .filter((m) => m?.info?.role === "assistant");
    for (const msg of assistantMessages) {
      const questionPart = (msg.parts || []).find(
        (p: any) =>
          p?.type === "tool" &&
          p?.tool === "question" &&
          p?.id &&
      ...
      if (questionPart) {
        maybeOpenQuestionModal(questionPart);
        return;
      }
    }
  };

  const handleQuestionResponse = async (selected: QuestionOption) => {
    if (!sessionId || !questionModal) return;
    setIsSubmittingQuestion(true);
    setError(null);
    try {
      if (questionModal.permissionID) {
        await RespondPermission(
          sessionId,
          questionModal.permissionID,
          selected.value,
        );
      } else {
        const mergedBody = {
          ...(questionModal.controlTemplate &&
          typeof questionModal.controlTemplate === "object"
            ? questionModal.controlTemplate
            : {}),
          response: selected.value,
          choice: selected.value,
          partID: questionModal.partID,
          messageID: questionModal.messageID,
          callID: questionModal.callID,
          tool: "question",
        };
        try {
          await SendTUIControlResponse(mergedBody);
        } catch {
          await SendTUIControlResponse(selected.value);
        }
      }

      handledQuestionPartIDsRef.current.add(questionModal.partID);
      setQuestionModal(null);
    } catch (err) {
      setError(`Failed to answer question tool: ${err}`);
    } finally {
  ...
  const syncPilotFromMessages = (msgs: any[]) => {
    ...
    if (trackedID) {
      assistantMsg =
        msgs.find(
          (m) =>
            (m as any)?.info?.id === trackedID &&
            (m as any)?.info?.role === "assistant",
        ) || null;
    }
    ...
    if (
      !assistantMsg &&
      pilotStatusRef.current === "pending" &&
      ...
      assistantMsg =
        [...msgs]
          ...
          .find(
            (m) =>
              (m as any)?.info?.role === "assistant" &&
              ((m as any)?.info?.time?.created ?? 0) >= cutoff,
          ) || null;
    ...
    if (!assistantMsg) {
      assistantMsg =
        [...msgs]
          .reverse()
          .find((m) => (m as any)?.info?.role === "assistant") || null;
    }
      ...
      const unsubscribe = EventsOn("server-event", (event: ServerEvent) => {
        ...
        if (event.type === "message.part.updated") {
          ...
          if (part.sessionID !== sessionId) {
            console.log(
              `Event sessionID ${part.sessionID} does not match current sessionId ${sessionId}, ignoring`,
            );
            return;
          }

          maybeOpenQuestionModal(part);
          ...
          setMessages((prevMessages) => {
            ...
            if (existingMsgIndex !== -1) {
              ...
              const partIndex = existingMessage.parts.findIndex(
                (p) => p.id === part.id,
              );
              if (partIndex !== -1) {
              ...
              console.log(
                `🔎 Part update for unknown message ${part.messageID}, waiting for message.updated`,
              );
              return prevMessages;
            }
          });
    ...
    if (sessionId) {
      ...
      GetMessages(sessionId)
        .then((msgs) => {
          ...
          scanPendingQuestionFromMessages(cleanedMessages as any[]);
        })
  ...
  const handleSendMessage = () => {
    ...
    const chatInput = models.ChatInput.createFrom({
      parts: [textPart],
      model: selectedModel
        ? models.ModelSelection.createFrom({
      ...
      .then((resp: any) => {
        ...
        GetMessages(sessionId)
          .then((msgs: any[]) => {
            ...
            scanPendingQuestionFromMessages(cleanedMessages);
          })
      ...
      {questionModal && (
        <div className="modal-backdrop">
          <div className="modal-content question-modal">
            <h3>Question Tool</h3>
            <p>{questionModal.title}</p>
            {questionModal.detail && (
              <p className="question-modal-detail">{questionModal.detail}</p>
            )}
            <div className="question-modal-actions">
              {questionModal.options.map((opt) => (
                <button
                  key={`${questionModal.partID}-${opt.value}`}
            ...
            <div key={index} className={`message ${msg.info.role}`}>
              {msg.parts.map((part, pIndex) => {
                ...
                if (part.type === "text" && typeof part.text === "string") {
                  console.log(
                    `📝 Rendering text part ${pIndex}, length: ${part.text.length}`,
                  );
                  return (
                  ...
                  return (
                    <div key={pIndex} className="part-reasoning">
                      <span className="part-label">💭 Thinking:</span>{" "}
                      {part.text}
                    </div>
                  );
                  ...
                  return (
                    <div
                      key={pIndex}
                      className={`part-tool part-tool-${status}`}
                    >
                      <span className="part-label">
                        {statusIcon} Tool [{toolPart.tool}]:
                      </span>{" "}
                      {title}
                    </div>
                  );
                ...
                if (
                  ["snapshot", "patch", "agent", "retry"].includes(part.type)
                ) {
                  return null;
                }

                console.warn(
                  `⚠️ Unknown part type "${part.type}" at index ${pIndex}`,
                  part,
                );
                return null;
              })}
      ...
      <div className="input-area">
        ...
        <div className="input-actions">
          <div style={{ display: "flex", gap: "8px", marginLeft: "auto" }}>
            ...
            {externalPilotStatus === "running" ||
            externalPilotStatus === "pending" ? (
              <button
