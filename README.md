...
- セッション管理（作成・切替・要約名へのリネーム・削除）
- モデル選択（プロバイダー別にグループ化）
- エージェント選択（ヘッダーからリアルタイム切替）
- LLM を利用した文章校正機能
- マークダウンレンダリングとコードシンタックスハイライト
- リアルタイムストリーミング表示（テキスト、思考過程、ツール実行状態）
...
- **セッション要約名の保存**: セッション右側の3点メニューから「要約」を選び、タイトルを編集
- **モデル変更**: ヘッダーのモデルドロップダウンから選択
- **エージェント変更**: ヘッダーのエージェントドロップダウンから選択（使用しない場合は "None" を選択）
